...
const StepFour: React.FC<StepOneProps> = ({ setNextSteps, setPrevSteps,changeRestaurantDetails ,restaurantInfo }) => {
  ...
  const uploadToS3 = async (state: string, city: string, reference_id:String,file: File,type: string) => {
...
const handleFinish = async () => {
  try {
    ...
    let reference_id;
    try{
      reference_id = await create_reference_id(restaurantInfo.name, restaurantInfo.gst);
    ...
    if (logo?.[0]) {
      ...
      logoKey = await uploadToS3(restaurantInfo.address.state, restaurantInfo.address.city,reference_id.reference_id, logo[0],type);
    }

    if (restaurantImage?.[0]) {
      const type = "Restaurant_Full_Image"
      imageKey = await uploadToS3(restaurantInfo.address.state, restaurantInfo.address.city,reference_id.reference_id, restaurantImage[0],type);
    }

    console.log({ logoKey, imageKey });
    console.log(restaurantInfo)
    const all_details = Object.assign(restaurantInfo,{full_img:imageKey , reference_id: reference_id.reference_id, logo_url:logoKey })
    ...
    <>
      <StepModal additionalClass="step-modal-new-width">
        ...
        <div className="Btn-cntr">
          ...
          <Btn onClick={() => handleFinish()}> {loading ? "Uploading..." : "Finish"}</Btn>
        </div>
      </StepModal>
    </>
  );
