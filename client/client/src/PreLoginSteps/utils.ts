...
export const create_reference_id = async(name: string, gst:string)=>{
    const response = await publicApi.post(base_api_url+ "restaurant/createReferenceID",{name, gst },{withCredentials:true})
...
export const upload_full_data = async(restaurant_info:PreloginDataInterface)=>{
    ...
    const response = await publicApi.post(base_api_url+ "restaurant/preloginInfo",restaurant_info,{withCredentials:true})
    if(response.status ===200){
        return response.data
