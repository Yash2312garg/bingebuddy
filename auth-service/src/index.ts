...
import restaurantAuthRoutes from "./routes/restaurant-auth/auth.routes";
...
import { AuthConsumer } from "./serivces/rabbitmq/consumers/auth.consumer";
const app = express();
...
app.use("/auth/v1/restaurant", restaurantAuthRoutes);
app.use("/auth/v1/restaurant/info",restaurantInfoRoutes);
app.get("/auth/v1/restaurant", (_req: Request, res: Response) => {
...
app.get('/auth/v1/health', (_req: Request, res: Response) => {
  ...
  return res.status(200).json({
    status: 'ok',
    message: 'Auth API server is healthy and running',
